	...
	return (
		<div className="dashboard__item">
			...
			<div className="dashboard__item--infor">
				...
				<div className="dashboard__item--infor-amountRoom">Địa chỉ:{item.address}</div>
				<div style={{ display: "flex", gap: "5px", alignItems: "center" }}>
					{renderStatus(item.status)}
					<div>-</div>
					<div className="dashboard__item--infor-amountRoom">
						Số phòng: {item.amountRooms}
					</div>
					<div>-</div>
					<div className="dashboard__item--infor-amountRoom">
						Số tầng: {item.numberOfFloors}
					</div>
				</div>
			</div>
			<div
				className="dashboard__item--action"
				onClick={() => setIsOpenAction(!isOpenListAction)}
			>
				<AppstoreOutlined />
			</div>
			{isOpenListAction && (
